 ...
 * A futuristic holographic workspace with floating windows,
 * question cards, charts, quiz analytics, students and AI generation.
 * Everything slowly moves; questions appear, charts animate, emails fly away.
 */
export default function CreatorStudioSection({ progress }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[520px] w-full max-w-5xl">
        ...
        <motion.div
          style={{ y: windowY }}
          className="absolute left-1/2 top-1/2 z-10 w-[300px] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-[#8B5CF6]/30 bg-[#0a0a1e]/80 p-4 shadow-[0_0_40px_rgba(139,92,246,0.2)] backdrop-blur-xl sm:w-[340px]"
        >
          <div className="flex items-center gap-2 border-b border-white/10 pb-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B5CF6]/20 text-[#C084FC]">
              <Wand2 className="h-4 w-4" />
            </div>
            <div>
              <div className="text-[12px] font-bold text-white">AI Quiz Generator</div>
              <div className="text-[10px] text-white/40">Generating questions…</div>
            </div>
            <motion.div
              ...
              >
                {q.done ? (
                  ...
                  <motion.div
                    ...
                    className="mt-0.5 h-3.5 w-3.5 shrink-0 rounded-full border-2 border-[#8B5CF6]/50"
                  />
                )}
                <span className="text-[11px] leading-relaxed text-white/70">{q.text}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div
          style={{ y: chartY }}
          className="absolute left-[6%] top-[18%] z-10 hidden w-[220px] rounded-2xl border border-white/10 bg-[#0a0a1e]/70 p-4 backdrop-blur-xl sm:block"
        >
          <div className="flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-[#EC4899]" />
            <span className="text-[11px] font-bold text-white">Quiz Analytics</span>
          </div>
          <div className="mt-3 flex h-20 items-end gap-1.5">
            {[35, 55, 42, 70, 58, 85, 65].map((h, i) => (
              <motion.div
                ...
                className="flex-1 rounded-t bg-gradient-to-t from-[#8B5CF6]/40 to-[#EC4899]/60"
              />
            ))}
          </div>
        ...
        <motion.div
          style={{ y: cardY }}
          className="absolute right-[6%] top-[22%] z-10 hidden w-[200px] rounded-2xl border border-white/10 bg-[#0a0a1e]/70 p-4 backdrop-blur-xl sm:block"
        >
          <div className="flex items-center gap-2">
            <FileText className="h-4 w-4 text-[#6366F1]" />
            <span className="text-[11px] font-bold text-white">Question Bank</span>
          </div>
          <div className="mt-3 space-y-2">
        ...
        <motion.div
          style={{ y: studentsY }}
          className="absolute bottom-[8%] left-[12%] z-10 hidden w-[180px] rounded-2xl border border-white/10 bg-[#0a0a1e]/70 p-3 backdrop-blur-xl sm:block"
        >
          <div className="flex items-center gap-2">
            <Users className="h-4 w-4 text-emerald-400" />
            <span className="text-[11px] font-bold text-white">Live Students</span>
          </div>
          <div className="mt-2 flex -space-x-2">
            {['#8B5CF6', '#EC4899', '#6366F1', '#F472B6', '#C084FC'].map((c, i) => (
              <motion.div
                ...
                className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#0a0a1e] text-[9px] font-bold text-white"
                style={{ background: c }}
              >
                {['A', 'R', 'S', 'M', 'K'][i]}
              </motion.div>
            ))}
            <div className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#0a0a1e] bg-white/10 text-[9px] font-bold text-white/60">
              +42
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="absolute right-[18%] top-[55%] z-20 flex items-center gap-2 rounded-xl border border-[#EC4899]/30 bg-[#0a0a1e]/80 px-3 py-2 backdrop-blur-xl"
        >
          <Mail className="h-3.5 w-3.5 text-[#EC4899]" />
          <span className="text-[10px] font-medium text-white/70">Results sent!</span>
        </motion.div>

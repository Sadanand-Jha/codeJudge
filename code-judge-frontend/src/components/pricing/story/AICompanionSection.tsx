...
function OrbitCard({
  ...
  return (
    <motion.button
      ...
      className="absolute z-10 flex h-28 w-28 flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md transition-colors duration-300 hover:border-white/25 sm:h-32 sm:w-32"
    ...
    >
      ...
      <span className="text-[11px] font-semibold text-white/80">{card.title}</span>
    </motion.button>
  )
}
 ...
 * A glowing orb that unfolds into 5 orbiting feature cards as the user scrolls.
 * Hovering enlarges a card; clicking reveals more info.
 */
export default function AICompanionSection({ progress }: Props) {
    ...
    >
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-sm text-white/50">
          A glowing intelligence that follows your coding journey.
        </p>
      </motion.div>
      ...
      <div className="relative flex h-[420px] w-[420px] items-center justify-center sm:h-[520px] sm:w-[520px]">
        ...
        >
          {/* Glow layers */}
          <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,rgba(139,92,246,0.5),transparent_70%)] blur-xl" />
          <div className="absolute inset-2 animate-pulse rounded-full bg-[radial-gradient(circle,rgba(236,72,153,0.35),transparent_70%)] blur-lg" />
          {/* Core */}
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
            className="relative flex h-24 w-24 items-center justify-center rounded-full border border-white/20 bg-gradient-to-br from-[#8B5CF6]/40 via-[#6366F1]/30 to-[#EC4899]/40 shadow-[0_0_60px_rgba(139,92,246,0.4)] sm:h-28 sm:w-28"
          >
            <div className="absolute inset-0 rounded-full border border-white/10" style={{ transform: 'rotate(45deg) scale(1.2)' }} />
            <div className="absolute inset-0 rounded-full border border-white/5" style={{ transform: 'rotate(-30deg) scale(1.4)' }} />
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            className="absolute bottom-[10%] z-30 w-full max-w-sm rounded-2xl border border-white/15 bg-[#0a0a1e]/90 p-5 backdrop-blur-xl"
          >
            {CARDS.filter((c) => c.id === selected).map((card) => {
              ...
              return (
                <div key={card.id}>
                  ...
                  <p className="mt-3 text-[13px] leading-relaxed text-white/60">{card.desc}</p>
                </div>
              )
            })}
          </motion.div>

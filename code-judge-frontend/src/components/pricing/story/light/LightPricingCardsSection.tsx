...
/**
 * Section 6 — Pricing Cards (Light Theme)
 * Premium glass cards emerge from the background with hover lift,
 * glow, mouse tilt, animated gradient border and reflection.
 */
export default function LightPricingCardsSection({ progress, onEnterBuilder }: Props) {
  ...
  return (
    ...
    >
          ...
          return (
            <motion.div
              ...
              className={`group relative rounded-3xl border p-6 backdrop-blur-xl transition-all duration-500 ${
                isUltimate
                  ? 'border-[rgba(139,92,246,0.3)] bg-gradient-to-b from-white/90 to-[#F3E8FF]/60 shadow-[0_8px_40px_rgba(139,92,246,0.15)]'
                  : 'border-[rgba(168,85,247,0.15)] bg-white/70 hover:border-[rgba(168,85,247,0.3)]'
              } ${isHovered ? 'scale-105 shadow-[0_12px_48px_rgba(168,85,247,0.2)]' : 'scale-100'}`}
            >
              ...
              {isHovered && (
                <motion.div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${plan.glow}, transparent 40%, ${plan.glow})`,
                    padding: '1px',
              ...
              >
                ...
                <div
                  ...
                  style={{ boxShadow: `0 4px 20px ${plan.glow}` }}
                >
                  <Icon className="h-6 w-6" />
                </div>

                {/* Plan name & price */}
                <h3 className="mt-4 text-lg font-extrabold text-[#1a1a2e]">{plan.name}</h3>
                <p className="text-[11px] text-[#8a8aa8]">{plan.tagline}</p>
                <div className="mt-3 flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-[#1a1a2e]">{plan.price}</span>
                  <span className="text-[11px] text-[#8a8aa8]">/month</span>
                </div>
                ...
                <ul className="mt-5 space-y-2.5">
                  {plan.features.map((feature, fi) => (
                    <motion.li
                      ...
                      className="flex items-center gap-2 text-[12px] text-[#4a4a6a]"
                    >
                      <Check
      ...
      <motion.button
        ...
        onClick={onEnterBuilder}
        className="relative z-10 mt-10 text-[12px] font-medium text-[#8a8aa8] transition-colors hover:text-[#7C3AED]"
      >

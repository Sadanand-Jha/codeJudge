...
import { BILLING_NOTIFICATIONS } from "./mockData";
import type { BillingNotification } from "./types";
import { IconButton } from "./ui";

const KIND_META: Record<BillingNotification["kind"], { icon: typeof Wallet; color: string }> = {
  payment: { icon: Wallet, color: "text-emerald-500" },
  payout: { icon: Banknote, color: "text-sky-500" },
  refund: { icon: RotateCcw, color: "text-amber-500" },
  statement: { icon: FileText, color: "text-violet-500" },
  verification: { icon: ShieldAlert, color: "text-rose-500" },
};

export function BillingNotifications() {
  const [open, setOpen] = useState(false);
  const [items, setItems] = useState(BILLING_NOTIFICATIONS);
  ...
  return (
    <div ref={ref} className="relative">
      <IconButton label="Financial notifications" active={open} onClick={() => setOpen((o) => !o)}>

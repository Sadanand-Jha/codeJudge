...
export function LiveRail({ onDownloadPdf, onStudentPreview, collapsed, onToggle }: { onDownloadPdf?: () => void; onStudentPreview?: () => void; collapsed?: boolean; onToggle?: () => void }) {
  ...
  if (collapsed) {
    return (
      <div className="hidden lg:flex w-10 shrink-0 rounded-xl border border-border bg-card flex-col items-center py-4">
        <button
          onClick={onToggle}
  ...
  return (
    <div className="flex h-full w-[340px] shrink-0 flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-6">
        <div>
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-bold uppercase tracking-wider text-text-primary">Question Properties</h3>
            <button onClick={() => setPropsCollapsed(!propsCollapsed)} className="text-text-muted hover:text-text-primary">
        ...
        <div>
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-text-muted">Question Status</h3>
        ...
        <div>
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-text-muted">Warnings</h3>
        ...
        <div className="pt-4 border-t border-border space-y-2">
          <label className="text-[11px] font-medium text-text-secondary">
            Topic
            <div className="mt-1 text-xs text-text-primary rounded-lg border border-border bg-card-hover px-2.5 py-2">
        ...
        <div className="rounded-xl border border-pink-200 dark:border-pink-400/30 bg-pink-50 dark:bg-pink-500/10 p-3">
          <p className="text-xs font-bold text-pink-700">Preview</p>
          <div className="mt-2 rounded-lg border border-pink-200 bg-card p-3">
            <p className="text-xs font-semibold text-pink-700">As Student</p>
            <p className="mt-1 text-xs leading-relaxed text-text-secondary">This is how the question will appear to students.</p>
            <button
              onClick={onStudentPreview}
              className="mt-2 inline-flex items-center gap-1 rounded-lg border border-pink-200 dark:border-pink-400/30 bg-card px-2.5 py-1 text-xs font-medium text-pink-600 dark:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-500/10"
            >
              👁 Preview
            </button>
          </div>
